...
public class Camera2BasicFragment extends Fragment
    ...
    public void onAttach(Context context) {
        ...
        if (context instanceof onPictureCapture) {
            mCallback = (onPictureCapture) context;
        } else {
            throw new RuntimeException(context.toString()
                    + " must implement Camera2BasicFragment's Interface");
        }
    }
    ...
    private void captureStillPicture() {
        try {
            Log.d(TAG,"inCaptureStillPicture");
            final Activity activity = getActivity();
            if (null == activity || null == mCameraDevice) {
                return;
            }
            mFile = new File(getActivity().getExternalFilesDir(null),
                    new SimpleDateFormat("yyyy.MM.dd-HH.mm.ss").format(Calendar.getInstance().getTime())+".jpg");
            ...
            mCallback.pictureCaptured(mFile.toString());
